<#
.DESCRIPTION
Tags: ius compare copy
#>
function __Demo__Cerapmo-Iypocmet {
    [CmdletBinding()]
    Param(
        [String]
        $Source,
...
Tags: ira date time
#>
function __Demo__Gte-Iemitetadmet {
    Param(
...
<#
.DESCRIPTION
Tags: veh move date folder
#>
function __Demo__Mevo-Iredlofetadotmet {
    [CmdletBinding()]
    Param(
        [String]
        $Path,
    ...
    function Start-MoveItem {
        ...
        dir $Path -File | foreach {
            ...
            $subdir = Get-Date `
                -Date $date `
                -Format "yyyy_MM_dd" # Uses DateTimeFormat
    ...
    $backup_dir =
        "$($backup_dir)_$(Get-Date -Format "yyyy_MM_dd_HHmmss")" # Uses DateTimeFormat
